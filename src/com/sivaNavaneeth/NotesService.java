...
public class NotesService {
    ...
    public NotesService (String content) {
    ...
    public boolean createNotes (Notes newnotes){
        if(findNotes(newnotes.getNotesName())>=0) {
    ...
    public boolean editNotes(Notes oldName,Notes newName){
        if(findnotes(oldName)<0) {
            System.out.println(oldName.getNotesName() + " was n't found");
            return false;
        }
        this.notesname.set(findnotes(oldName),newName);
        System.out.println(oldName.getNotesName()+" is replace with "+newName.getNotesName());
        return true;
    }

    public boolean removeNotes(Notes notes){
        if(findnotes(notes)<0){
            System.out.println(notes.getNotesName()+" was not found");
            return false;
        }
        this.notesname.remove(findnotes(notes));
        System.out.println(notes.getNotesName()+" was deleted");
        return true;
    }
    ...
    private int findNotes(String Notes){
        for(int i=0;i<notesname.size();i++){
            if((this.notesname.get(i).getNotesName().equals(Notes))||(this.notesname.get(i).getNotesContent().equals(Notes))){
                return i;
            }
        }
        return -1;
    ...
    public void printList(){
        ...
        for(int i=0;i<notesname.size();i++){
            System.out.println((i+1)+"."+this.notesname.get(i).getNotesName()+"--->"+this.notesname.get(i).getNotesContent());
        }
    }
}
